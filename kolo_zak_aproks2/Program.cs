...
namespace kol_zal_aproks
{
    class Program
    {
        ...
        {
            var odleglosci = new List<KeyValuePair<double, List<double>>>();
            foreach (var probka in probkiWzorcowe)
            {
                var odl = 0d;

                for (int i = 0; i < liczbaAtrybutowWejsciowych; i++)
                {
                    odl += Math.Pow(probkaTestowa[i] - probka[i], 2);
                }

                odleglosci.Add(new KeyValuePair<double, List<double>>(odl, probka));
            }

            var najblizsze = odleglosci.OrderByDescending(x => x.Key).ToList();

            double licznik = 0;
            for (int i = 0; i < k; i++)
            {
                licznik += najblizsze[i].Value[probkaTestowa.Count] * Math.Pow(1.5, k);
            }
            double mianownik = 0;
            for (int i = 0; i < k; i++)
            {
                mianownik += Math.Pow(1.5, k);
            }
